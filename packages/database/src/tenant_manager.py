...
# Tenant-scoped tables that admin/migration helpers are allowed to query.
# This list is the authoritative allow-list used to defuse f-string
# interpolation of table names into SQL: any table name that is not in
# this set is rejected before it reaches asyncpg. Keeping it as a
# `frozenset` makes accidental mutation impossible at runtime.
...
def _assert_known_table(table: str) -> str:
    """Return `table` if it is in the tenant-scoped allow-list, else raise.

    This function is the ONLY place in this module that is allowed to
    interpolate a table name into an SQL string. Every caller must run
    its identifier through here first so that a future refactor cannot
    accidentally open an injection vector by widening the source of the
    identifier (e.g. passing it in from an HTTP request).
    """
    if table not in _TENANT_SCOPED_TABLES:
        raise ValueError(
            f"Refusing to run query against unknown table {table!r}; "
            f"allowed: {sorted(_TENANT_SCOPED_TABLES)}"
        )
    return table
...
class TenantManager:
    ...
    async def with_tenant(
        ...
        async with self._pool.acquire() as conn:
            ...
            await conn.execute("SET LOCAL app.current_tenant = $1", str(tenant.id))
    ...
    async def _verify_rls_context(
        self, conn: asyncpg.Connection, tenant_id: uuid.UUID
    ) -> None:
        ...
        if current_tenant != str(tenant_id):
            raise RuntimeError(
                f"RLS context not set properly. Expected: {tenant_id}, Got: {current_tenant}"
            )
    ...
    async def get_tenant_stats(self, tenant_id: str) -> Dict[str, int]:
        ...
        async with self.with_tenant(tenant_id) as conn:
            ...
            for table in _TENANT_SCOPED_TABLES:
                safe_table = _assert_known_table(table)
    ...
    async def migrate_existing_data(self, tenant_id: str) -> None:
        ...
        async with self._pool.acquire() as conn:
            ...
            for table in _TENANT_SCOPED_TABLES:
                safe_table = _assert_known_table(table)
                await conn.execute(
                    f"UPDATE {safe_table} SET tenant_id = $1 WHERE tenant_id IS NULL",
                    tenant_id,
                )
