...
impl fmt::Display for BacktestError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            BacktestError::MissingPriceSeries(symbol) => {
                write!(f, "Missing price series for {}", symbol)
            }
...
fn gen_prices(base: f64, vol: f64, n: usize, seed: u64) -> Vec<f64> {
    ...
    for _ in 1..n {
        ...
        let drift = 0.0;
        let ret = drift * dt + vol * z * (dt as f64).sqrt();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "Period: 2026-02-20 to 2026-05-02 | Balance: ${:.2} CAD\n",
        balance_cad
    );
    ...
    let bars = days * 78;
    println!(
        "Pre-generating {} bars across {} assets...",
        bars,
        assets.len()
    );
    ...
    for (idx, asset) in assets.iter().enumerate() {
        ...
        price_series.insert(
            asset.symbol.clone(),
            gen_prices(asset.base_price, asset.vol, bars, seed),
        );
    ...
    while current_time < end {
        ...
        for ot in open_trades {
            ...
            let elapsed = current_time
                .signed_duration_since(ot.entry_time)
                .num_minutes();
            let mut exited = false;
            let mut reason = String::new();
            let mut exit_price = p;
            if ot.direction == 1 && p >= ot.target {
                exited = true;
                reason = "target".into();
            } else if ot.direction == 1 && p <= ot.stop {
                exited = true;
                reason = "stop".into();
                exit_price = ot.stop;
            } else if ot.direction == -1 && p <= ot.target {
                exited = true;
                reason = "target".into();
            } else if ot.direction == -1 && p >= ot.stop {
                exited = true;
            ...
            if exited {
                ...
                let class = assets
                    .iter()
                    .find(|a| a.symbol == ot.symbol)
                    .map(|a| a.class.clone())
                    .unwrap_or_default();
                closed_trades.push(ClosedTrade {
                    symbol: ot.symbol.clone(),
                    class,
        ...
        if bar_idx > 20 {
            ...
            for asset in &assets {
                ...
                let high = p * (1.0 + asset.spread_bps / 20000.0);
                let low = p * (1.0 - asset.spread_bps / 20000.0);
                fabric_assets.insert(
                    asset.symbol.clone(),
                    AssetFabricState {
                        symbol: asset.symbol.clone(),
                        price: p,
                        volume_24h: volume_series[&asset.symbol]
                            [bar_idx.min(volume_series[&asset.symbol].len() - 1)]
                            * 1000.0,
                        volatility_short: asset.vol * p,
                        volatility_long: asset.vol * p * 0.8,
                        liquidity_score: 1.0,
            ...
            let fabric = FabricState {
                ...
                asset_states: fabric_assets,
                global_regime: MarketRegime::RangeBound,
                noise_level: 0.0,
                predictability_index: 0.5,
                deterministic_hash: format!("hash_{}", current_time.timestamp()),
            };
            ...
            for gr in result.allowed {
                ...
                if open_trades.iter().any(|t| t.symbol == sym) {
                    continue;
                }
        ...
        let open_value: f64 = open_trades
            .iter()
            .map(|t| {
        ...
        if bar_idx % 78 == 0 {
            equity_snapshots.push((current_time, equity));
        }
    ...
    let lose_trades: Vec<&ClosedTrade> =
        closed_trades.iter().filter(|t| t.pnl_cad <= 0.0).collect();
    let win_rate = if !closed_trades.is_empty() {
        win_trades.len() as f64 / closed_trades.len() as f64 * 100.0
    } else {
        0.0
    };
    let gross_profit: f64 = win_trades.iter().map(|t| t.pnl_cad).sum();
    let gross_loss: f64 = lose_trades.iter().map(|t| t.pnl_cad).sum();
    let profit_factor = if gross_loss.abs() > 0.01 {
    ...
    println!("  ────────────────────────────────────────────────────────────────");
    println!(
        "  Patterns: {:>6} | Fused: {:>6} | Decisions: {:>6} | Allowed: {:>6} | Rejected: {:>6}",
    ...
    md.push_str(
        "| # | Symbol | Class | Dir | Entry | Exit | Qty | P&L (CAD) | Fee | Reason | Date |\n",
    );
    md.push_str(
        "|---|--------|-------|-----|-------|------|-----|-----------|-----|--------|------|\n",
    );
