...
// ============================================================================
// NOTE: Mem0MemoryImprint, Mem0RetrievalEvent, Mem0Telemetry are now
// re-exported from `traderx_mem0_types` for cross-crate/ws consistency.
// See: /packages/traderx-mem0-types/src/lib.rs for canonical definitions.
// ============================================================================
...
impl EventJournal {
    ...
    pub async fn append(&self, mut entry: JournalEntry) -> Result<(), JournalError> {
        ...
        conn.set_ex::<String, String, usize>(
            key,
            serialized,
            self.config.retention_seconds as usize,
        )
        .await
        .map_err(|e| JournalError::Redis(e.to_string()))?;
        ...
        let aggregate_key = format!(
            "{}:aggregate:{}",
            self.config.key_prefix, entry.aggregate_id
        );
        conn.zadd::<_, _, _, ()>(
            &aggregate_key,
            entry.sequence.to_string(),
            entry.sequence as f64,
        )
        .await
        .map_err(|e| JournalError::Redis(e.to_string()))?;

        // Set expiration on aggregate index
        conn.expire::<String, usize>(aggregate_key, self.config.retention_seconds as usize)
            .await
            .map_err(|e| JournalError::Redis(e.to_string()))?;

        info!(
            "Appended journal entry: {} (seq: {})",
    ...
    pub async fn get_events(
        ...
        let sequences: Vec<String> = if let Some(from) = from_sequence {
            conn.zrangebyscore(&aggregate_key, from as f64, "+inf")
                .await
                .map_err(|e| JournalError::Redis(e.to_string()))?
        } else {
            conn.zrange(&aggregate_key, 0, -1)
                .await
                .map_err(|e| JournalError::Redis(e.to_string()))?
        };
    ...
    pub async fn get_events_from(
        ...
        let to_sequence = limit
            .map(|l| from_sequence + l as u64 - 1)
            .unwrap_or(max_seq);
    ...
    pub async fn create_snapshot<T>(
        &self,
        aggregate_id: Uuid,
        state: &T,
        sequence: u64,
    ...
    {
        let serialized =
            serde_json::to_string(state).map_err(|e| JournalError::Serialization(e.to_string()))?;
        ...
        info!(
            "Created snapshot for aggregate {} at sequence {}",
    ...
    pub async fn get_snapshot<T>(
        &self,
        aggregate_id: Uuid,
    ) -> Result<Option<(T, u64)>, JournalError>
    where
        T: for<'de> Deserialize<'de>,
    {
        ...
        let snapshot_result: Result<Option<String>, redis::RedisError> =
            conn.get::<String, Option<String>>(snapshot_key).await;
        let sequence_result: Result<Option<u64>, redis::RedisError> =
            conn.get::<String, Option<u64>>(seq_key).await;
    ...
    pub async fn append_mem0_imprint(
        &self,
    ...
    pub async fn append_mem0_retrieval(
        &self,
        retrieval: &Mem0RetrievalEvent,
    ) -> Result<(), JournalError> {
    ...
    pub async fn append_mem0_telemetry(
        &self,
        telemetry: &Mem0Telemetry,
    ) -> Result<(), JournalError> {
    ...
    pub async fn get_mem0_imprints(
        &self,
        session_id: Uuid,
    ...
    pub async fn get_mem0_telemetry(
        &self,
        session_id: Uuid,
    ) -> Result<Vec<Mem0Telemetry>, JournalError> {
        let events = self.get_events(session_id, None).await?;
    ...
    pub async fn get_mem0_events(
        ...
        let mem0_events: Vec<JournalEntry> = events
            ...
            .filter(|e| {
                matches!(
                    e.event_type.as_str(),
                    "Mem0MemoryImprint" | "Mem0Retrieval" | "Mem0Telemetry"
                )
            })
            .collect();
...
mod tests {
    ...
    async fn test_mem0_imprint_append_and_retrieve() {
        ...
        // Create mem0 memory imprint using shared builder
        let imprint = Mem0MemoryImprint::new("agent_qa", "Test memory content", session_id)
            .with_agent_role("SystemsArchitect")
            .with_category("Architecture")
            .with_confidence(0.95)
            .with_tags(vec!["test".to_string(), "qa".to_string()])
        ...
        // Append mem0 telemetry using shared builder
        let telemetry = Mem0Telemetry::new(session_id)
            .with_totals(11, 5)
            .with_hit_rate(0.83)
            .with_latency_ms(1.5)
            .with_distribution(json!({"agent_qa": 11}));
