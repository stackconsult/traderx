...
class Evaluator(object):
    ...
    def evaluate(
        ...
        P_topk = self.pred.detach().cpu().numpy().copy()
        ...
        for i_class in range(self.n_class):
            ...
            class_metric_dict[i_class] = class_metric
    ...
    def get_score_class(self, i_class, threshold_type='threshold'):
        ...
        if threshold_type == 'threshold':
            pred = self.pred[:,i_class]

        else:
            pred = self.pred_topk[:,i_class]
        gt_label = self.gt_label[:,i_class]
...
def preprocess_pos_neg(targets):
    """
    Preprocess positive and negative attribute indices to create ground truth labels.

    Args:
        targets (list of dict): List of target dictionaries, each containing:
            - 'pos_att_classes' (torch.Tensor): Positive attribute indices (binary format).
            - 'neg_att_classes' (torch.Tensor): Negative attribute indices (binary format).

    Returns:
        list: List of ground truth labels where:
              1 indicates positive,
              0 indicates negative,
              2 indicates neutral/unlabeled.
    """
    pos_atts = torch.cat([target['pos_att_classes'] for target in targets], dim=0)
    neg_atts = torch.cat([target['neg_att_classes'] for target in targets], dim=0)

    gts = []
    for pos_att, neg_att in zip(pos_atts, neg_atts):
        gt = 2 * np.ones(pos_att.unsqueeze(0).shape, dtype=np.int64)
        gt[:, pos_att.cpu() == 1] = np.int64(1)
        gt[:, neg_att.cpu() == 1] = np.int64(0)
        gts.extend(gt)
