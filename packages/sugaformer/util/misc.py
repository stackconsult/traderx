...
def collate_fn(batch):
    ...
    if len(batch) == 0:
        # Return an empty batch if no valid samples are found
        return None
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    if tensor_list[0].ndim == 3:
        ...
        b, _, h, w = tensor_list[0].shape
        dtype = tensor_list[0].dtype
        device = tensor_list[0].device
        tensor = torch.stack(tensor_list, dim=0).to(device)  # Combine along batch dimension
        mask = torch.zeros((len(tensor_list), h, w), dtype=torch.bool, device=device)  # No mask for batch
    else:
